...
BEGIN
    RETURN QUERY (
        WITH
            ...
            group_selection AS (
                ...
                FROM (
                    ...
                    WHERE 
                        ...
                        group_stability_index < max_consumption_stab_index ) rg
            ...
            sku_with_max_margin AS (
                ...
                FROM (
                    ...
                    JOIN stores sp ON sp.transaction_store_id = c.customer_primary_store ) pm
                ORDER BY 
                    pm.customer_id,
                    pm.group_id,
                    pm.store_id,
        ...
        FROM sku_with_max_margin swmm
        LEFT JOIN product_grid pr ON pr.sku_id = swmm.sku_id
        ...
        WHERE
            (st.transactions_count::NUMERIC / gt.group_transactions_count) < 
                (max_sku_share_percentage / 100) AND 
            (((allowable_margin_share_percentage / 100) * swmm.max_margin::NUMERIC) / swmm.sku_retail_price) <
                round_discount(p.group_min_discount) );
END; $$
LANGUAGE plpgsql;
