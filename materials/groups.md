...
1.  **Формирование списка SKU для клиента.** Для каждого клиента (для
    ...
    содержащиеся в полях `Transaction_ID` таблиц [Чеки](../README.md#таблица-чеки) и [Транзакции](../README.md#таблица-транзакции), `Customer_Card_ID` таблиц [Транзакции](../README.md#таблица-транзакции) и [Персональные данные](../README.md#таблица-персональные-данные), `Customer_ID` таблицы [Персональные данные](../README.md#таблица-персональные-данные).
...
5.  **Подсчет финансовых показателей по группе.** Для каждого клиента по
    ...
    -   Себестоимость купленного клиентом товара в течение
        анализируемого периода. Суммируются данные из поля `SKU_Cost`
...
6. **Определение общего количества транзакций клиента.** Определяется
    ...
    `First_Group_Transaction_Date` таблицы [Периоды](../README.md#представление-периоды)) и меньше или
    равна дате последней транзакции клиента с группой (значение поля
    `Last_Group_Transaction_Date` таблицы [Периоды](../README.md#представление-периоды)).
...
8. **Подсчет давности приобретения группы.** Из текущей даты вычитается
    ...
    клиентом выбирается максимальное значение по полю `Transaction_Date`
...
10. **Расчет интервалов потребления группы.** Определяются все интервалы
    ...
    совершения (значению поля `Transaction_Date` таблицы [История покупок](../README.md#представление-история-покупок)) от самой ранней к самой поздней. Из даты каждой
...
14. **Выбор метода расчета маржи.** По умолчанию маржа рассчитывается
    ...
    -   В случае выбора метода расчета маржи по периоду пользователь
        указывает, за какое количество дней от текущей даты в обратном
        ...
        `Transaction_Date` таблицы [История покупок](../README.md#представление-история-покупок).
    ...
    -   В случае выбора метода расчета маржи по количеству транзакций
        ...
        содержащиеся в поле `Transaction_Date` таблицы История
        покупок`.
...
15. **Расчет фактической маржи по группе.** Для определения фактической
    ...
    таблицы [История покупок](../README.md#представление-история-покупок)), после чего делится на сумму, на
    которую был куплен товар (поле `Group_Summ_Paid` таблицы [История покупок](../README.md#представление-история-покупок)). Итоговое значение сохраняется в качестве
